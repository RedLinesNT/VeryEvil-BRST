Shader "Very Evil/Cel Shading/Cel Shaded Grass" {
	...
	SubShader {
		...
		HLSLINCLUDE
			...
			// Simple noise function, returns a number in the 0...1 range.
			...
			// This is a test version of the tessellation that takes distance from the viewer into account.
			...
			// The patch constant function is where we create new control points on the patch.
			// For the edges, increasing the tessellation factors adds new vertices on the edge.
			// Increasing the inside will add more 'layers' inside the new triangle.
			...
			// It operates on each patch (in our case, a patch is a triangle), and outputs new control points for the other tessellation stages.
			// The patch constant function is where we create new control points (which are kind of like new vertices).
			[domain("tri")] [outputcontrolpoints(3)] [outputtopology("triangle_cw")] [partitioning("integer")] [patchconstantfunc("patchConstantFunc")] VertexInput hull(InputPatch<VertexInput, 3> patch, uint id : SV_OutputControlPointID) {
				return patch[id];
			}
			...
			[domain("tri")] VertexOutput domain(TessellationFactors factors, OutputPatch<VertexInput, 3> patch, float3 barycentricCoordinates : SV_DomainLocation) {
				VertexInput i;
			...
			// This is the geometry shader. For each vertex on the mesh, a leaf blade is created by generating additional vertices.
			[maxvertexcount(BLADE_SEGMENTS * 2 + 1)] void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream) {
        ...
        Pass {
			...
			// The lighting sections of the frag shader.
